...
export default function DashboardPage() {
    ...
    const [summaryStats, setSummaryStats] = useState({ totalRevenue: 0, totalTransactions: 0 });
    ...
    const fetchData = useCallback(async <T,>(
        ...
        try {
          ...
          if (Array.isArray(dataToSet)) {
            ...
            console.warn(`Data dari ${url} bukan array, mengeset ke array kosong.`);
            setter([]);
          }
        } catch (e: any) {
          setError(e.message);
    ...
    const fetchProducts = useCallback(() => fetchData<Product>('/api/products?limit=1000', setProducts), [fetchData]);
    const fetchCategories = useCallback(() => fetchData<Category>('/api/category', setCategories), [fetchData]);
    const fetchOrders = useCallback(() => fetchData<Order>('/api/orders', setOrders), [fetchData]);
    const fetchStockLogs = useCallback(() => fetchData<StockLog>('/api/stocklog', setStockLogs), [fetchData]);
    const fetchUsers = useCallback(() => fetchData<User>('/api/users', setUsers), [fetchData]);
    
    const fetchDashboardSummary = useCallback(async () => {
        try {
            const response = await fetch('/api/dashboard/summary');
            if (!response.ok) throw new Error('Gagal mengambil ringkasan dashboard.');
            const result: ApiResponse<{ summary: any; salesTrend: SalesDataPoint[]; topProducts: TopProductDataPoint[] }> = await response.json();
            if (result.data) {
    ...
    useEffect(() => {
        ...
        const loadDataForPage = async () => {
            ...
            setError(null);
            switch (currentPage) {
                case 'dashboard': await fetchDashboardSummary(); break;
                case 'products': await Promise.all([fetchProducts(), fetchCategories()]); break;
                case 'transactions': await fetchOrders(); break;
                case 'sales_report': await fetchOrders(); break;
                case 'stock_logs': await fetchStockLogs(); break;
        ...
        loadDataForPage();
    }, [currentPage, status, fetchProducts, fetchCategories, fetchOrders, fetchStockLogs, fetchUsers, fetchDashboardSummary]);
    ...
    const renderPageContent = () => {
        ...
        switch (currentPage) {
            case 'dashboard':
                return <DashboardView salesSummary={salesSummary} topProducts={topProducts} totalRevenue={summaryStats.totalRevenue} totalTransactions={summaryStats.totalTransactions} />;
            case 'products':
