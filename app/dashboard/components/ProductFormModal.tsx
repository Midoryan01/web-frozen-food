...
const ProductFormModal: React.FC<ProductFormModalProps> = ({ product, categories, onClose, onSave, apiBaseUrl }) => {
  ...
  const [currentImageUrl, setCurrentImageUrl] = useState<string | null>(product?.imageUrl || null); // Untuk menampilkan gambar yang ada
  const [imagePreview, setImagePreview] = useState<string | null>(product?.imageUrl || null); // Untuk preview gambar baru
  ...
  useEffect(() => {
    if (product) {
      ...
      setFormData({
        name: '', sku: '', buyPrice: 0, sellPrice: 0, stock: 0, description: '', supplier: '', expiryDate: '', categoryId: '',
      });
  ...
  const handleSubmit = async (e: FormEvent<HTMLFormElement>) => {
    ...
    try {
      const url = product ? `${apiBaseUrl}/products/${product.id}` : `${apiBaseUrl}/products`;
      ...
      if (!response.ok) {
        const errorData = await response.json().catch(() => ({ message: `Gagal ${product ? 'memperbarui' : 'menambah'} produk. Respons server tidak valid.` }));
        throw new Error(errorData.message || `Gagal ${product ? 'memperbarui' : 'menambah'} produk`);
      }
  ...
  return (
    <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center p-4 z-50 print:hidden">
      <div className="bg-white p-6 sm:p-8 rounded-xl shadow-2xl w-full max-w-lg max-h-[95vh] flex flex-col">
        ...
        <form onSubmit={handleSubmit} className="space-y-4 overflow-y-auto pr-2 custom-scrollbar flex-grow">
          {/* Nama Produk */}
          <div>
            <label htmlFor="name" className="block text-sm font-medium text-slate-700 mb-1">Nama Produk*</label>
            <input type="text" name="name" id="name" value={formData.name} onChange={handleChange} required className="w-full p-2.5 border border-slate-300 rounded-lg focus:ring-2 focus:ring-sky-500 focus:border-sky-500 transition-shadow" />
          </div>

          {/* SKU */}
          <div>
            <label htmlFor="sku" className="block text-sm font-medium text-slate-700 mb-1">SKU (Opsional)</label>
            <input type="text" name="sku" id="sku" value={formData.sku} onChange={handleChange} className="w-full p-2.5 border border-slate-300 rounded-lg focus:ring-2 focus:ring-sky-500 focus:border-sky-500 transition-shadow" />
          </div>
          
          {/* Harga Beli & Jual */}
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div>
              <label htmlFor="buyPrice" className="block text-sm font-medium text-slate-700 mb-1">Harga Beli*</label>
              <input type="number" name="buyPrice" id="buyPrice" value={formData.buyPrice} onChange={handleChange} required min="0" step="any" className="w-full p-2.5 border border-slate-300 rounded-lg focus:ring-2 focus:ring-sky-500 focus:border-sky-500 transition-shadow" />
            </div>
            <div>
              <label htmlFor="sellPrice" className="block text-sm font-medium text-slate-700 mb-1">Harga Jual*</label>
              <input type="number" name="sellPrice" id="sellPrice" value={formData.sellPrice} onChange={handleChange} required min="0" step="any" className="w-full p-2.5 border border-slate-300 rounded-lg focus:ring-2 focus:ring-sky-500 focus:border-sky-500 transition-shadow" />
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div>
              <label htmlFor="stock" className="block text-sm font-medium text-slate-700 mb-1">Stok Awal/Saat Ini*</label>
              <input type="number" name="stock" id="stock" value={formData.stock} onChange={handleChange} required min="0" className="w-full p-2.5 border border-slate-300 rounded-lg focus:ring-2 focus:ring-sky-500 focus:border-sky-500 transition-shadow" />
            </div>
            <div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div>
              <label htmlFor="expiryDate" className="block text-sm font-medium text-slate-700 mb-1">Tanggal Kadaluwarsa*</label>
              <input type="date" name="expiryDate" id="expiryDate" value={formData.expiryDate} onChange={handleChange} required className="w-full p-2.5 border border-slate-300 rounded-lg focus:ring-2 focus:ring-sky-500 focus:border-sky-500 transition-shadow" />
            </div>
             <div>
              <label htmlFor="supplier" className="block text-sm font-medium text-slate-700 mb-1">Supplier (Opsional)</label>
              <input type="text" name="supplier" id="supplier" value={formData.supplier} onChange={handleChange} className="w-full p-2.5 border border-slate-300 rounded-lg focus:ring-2 focus:ring-sky-500 focus:border-sky-500 transition-shadow" />
            </div>
          </div>
          ...
          <div>
            ...
            <textarea name="description" id="description" value={formData.description} onChange={handleChange} rows={3} className="w-full p-2.5 border border-slate-300 rounded-lg focus:ring-2 focus:ring-sky-500 focus:border-sky-500 transition-shadow"></textarea>
          </div>
            ...
            <div className="mt-1 flex items-center gap-4">
                {imagePreview ? (
                    <div className="relative group">
                        <Image src={imagePreview} alt="Preview Produk" width={80} height={80} className="rounded-md object-cover border border-slate-300" />
                        <button type="button" onClick={handleRemoveImage} title="Hapus Gambar" className="absolute -top-2 -right-2 bg-red-500 text-white rounded-full p-0.5 opacity-0 group-hover:opacity-100 transition-opacity">
                            <XCircle size={18}/>
                        </button>
                    </div>
                ) : (
                ...
                <label htmlFor="image-upload-input" className="cursor-pointer bg-white py-2 px-3 border border-slate-300 rounded-md shadow-sm text-sm leading-4 font-medium text-slate-700 hover:bg-slate-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-sky-500 flex items-center gap-2">
                    <UploadCloud size={16} />
                    <span>{imageFile || currentImageUrl ? 'Ganti Gambar' : 'Unggah Gambar'}</span>
                </label>
                <input id="image-upload-input" name="image" type="file" onChange={handleImageChange} accept="image/*" className="sr-only" />
            </div>
            <p className="text-xs text-slate-500 mt-1">Max. 2MB (JPG, PNG, WEBP)</p>
          </div>
          
          <div className="flex justify-end gap-3 pt-6 border-t border-slate-200 mt-6">
            <button type="button" onClick={onClose} className="px-5 py-2.5 text-sm font-medium text-slate-700 bg-slate-100 hover:bg-slate-200 rounded-lg transition-colors">
              Batal
            </button>
            <button 
              type="submit" 
