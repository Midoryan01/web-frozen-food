...
import Head from 'next/head'; // Diperlukan untuk menambahkan script library
import Script from 'next/script'; // Diperlukan untuk menambahkan script library
...
const SalesReportView: React.FC<{ orders: Order[] }> = ({ orders }) => {
    ...
    const handleExportToExcel = () => {
        ...
        // 3. Tambahkan baris total di akhir
        XLSX.utils.sheet_add_aoa(worksheet, [
            [], // Baris kosong sebagai pemisah
            ["", "", "", "", "", "", "TOTAL", totalRevenue, totalProfit]
        ], { origin: -1 }); // -1 artinya menambahkan di akhir

        // 4. Buat workbook baru dan tambahkan worksheet
        const workbook = XLSX.utils.book_new();
        XLSX.utils.book_append_sheet(workbook, worksheet, "Laporan Penjualan");

        // 5. Buat nama file dan unduh
    ...
    return (
        <>
            {/* Menambahkan library xlsx ke halaman */}
            <Script
            ...
            <div className="space-y-6">
                <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                    ...
                    <button
                        ...
                        className="flex items-center justify-center gap-2 px-4 py-2 bg-green-600 text-white rounded-lg shadow-sm hover:bg-green-700 transition-colors text-sm font-medium"
                    >
                        <FileDown size={16} />
                        Cetak Excel
                    </button>
