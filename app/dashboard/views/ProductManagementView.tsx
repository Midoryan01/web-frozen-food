        ...
        <div className="space-y-6">
            <div className="flex flex-col sm:flex-row justify-between items-center gap-4">
                ...
                <div className="flex gap-2">
                    ...
                    <button onClick={handleAddProduct} className="bg-sky-600 text-white px-4 py-2.5 rounded-lg hover:bg-sky-700 transition-colors flex items-center gap-2 text-sm font-medium">
            ...
            {isLoadingTable && filteredAndSortedProducts.length === 0 ? (
                <div className="flex justify-center items-center py-10">
                    <div className="animate-spin rounded-full h-12 w-12 border-t-2 border-b-2 border-sky-600"></div><p className="ml-3 text-slate-600">Memuat produk...</p>
                </div>
            ) : !isLoadingTable && filteredAndSortedProducts.length === 0 ? (
                            ...
                            {filteredAndSortedProducts.map(product => (
                                <tr key={product.id} className="hover:bg-sky-50/50 transition-colors duration-150">
                                    ...
                                    <td className="p-3 text-center align-top">
                                        <button onClick={() => handleEditProduct(product)} title="Edit Produk" className="text-sky-600 hover:text-sky-800 p-1.5 rounded-md hover:bg-sky-100 transition-colors"><Edit3 size={16} /></button>
                                        <button onClick={() => handleDeleteProduct(product.id)} title="Hapus Produk" className="text-red-500 hover:text-red-700 p-1.5 rounded-md hover:bg-red-100 transition-colors ml-1"><Trash2 size={16} /></button>
                                    </td>
                                </tr>
                            ))}
                        </tbody>
