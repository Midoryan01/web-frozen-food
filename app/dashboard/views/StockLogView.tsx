...
import type { StockLog } from '@/types/index';
import {
  RefreshCw,
  Search,
  ArrowUpCircle,
        ...
        <form onSubmit={handleSubmit}>
          ...
          <div className="flex justify-end gap-3 mt-4">
            ...
            <button
              type="submit"
              disabled={isSaving}
              className="px-4 py-2 bg-sky-600 text-white rounded-lg hover:bg-sky-700 disabled:bg-slate-400"
            >
...
const StockLogView: React.FC<StockLogViewProps> = ({
  stockLogs,
  isLoading,
  fetchStockLogs,
}) => {
  ...
  const handleExportToExcel = () => {
    ...
    const dataToExport = filteredLogs.map((log) => ({
      ...
      'Stok Awal': 100, // dummy
      Perubahan: log.quantity,
      'Total Stok': 100 + log.quantity, // dummy
    ...
    XLSX.utils.book_append_sheet(wb, ws, 'Log Stok');
    XLSX.writeFile(wb, `Laporan_Log_Stok_${new Date().toISOString().split('T')[0]}.xlsx`);
  };
  ...
  const handleDeleteLog = async (logId: number) => {
    if (
      window.confirm(
        'Apakah Anda yakin ingin menghapus log ini? Stok akan dikembalikan ke kondisi semula.'
      )
    ) {
      try {
        const response = await fetch(`/api/stocklog/${logId}`, {
  ...
  // Fungsi untuk menghitung stok sebelum log ini
    const getStockBefore = (log: StockLog, allLogs: StockLog[]) => {
  return allLogs
    .filter(
      l =>
    ...
    <>
      ...
      <div className="space-y-6">
        ...
        <div className="p-4 bg-white rounded-lg shadow-sm border border-slate-200 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
          <div>
            <label htmlFor="searchProduct" className="block text-xs font-medium text-slate-600 mb-1">
              Cari Produk
            </label>
          ...
          <div>
            <label htmlFor="filterChangeType" className="block text-xs font-medium text-slate-600 mb-1">
              Perubahan
            </label>
            <select
              id="filterChangeType"
          ...
          <div>
            <label htmlFor="filterLogType" className="block text-xs font-medium text-slate-600 mb-1">
              Tipe Log
            </label>
            <select
              id="filterLogType"
          ...
          <div className="grid grid-cols-2 gap-2">
            <div>
              <label htmlFor="startDateLog" className="block text-xs font-medium text-slate-600 mb-1">
                Dari
              </label>
              <input
                type="date"
            ...
            <div>
              <label htmlFor="endDateLog" className="block text-xs font-medium text-slate-600 mb-1">
                Sampai
              </label>
              <input
                type="date"
            ...
            <table className="w-full min-w-[1000px]">
              <thead className="bg-slate-100">
                <tr>
                    ...
                    <th className="p-3 text-left text-xs font-semibold text-slate-500 uppercase">Supplier</th>
                    <th className="p-3 text-right text-xs font-semibold text-slate-500 uppercase">Stok Awal</th>
                    <th className="p-3 text-right text-xs font-semibold text-slate-500 uppercase">Perubahan</th>
                    <th className="p-3 text-right text-xs font-semibold text-slate-500 uppercase">Total Stok</th>
                ...
                {paginatedLogs.map((log) => {
                    const stokAwal = getStockBefore(log, stockLogs);
                    const totalStok = stokAwal + log.quantity;

                    return (
                    <tr key={log.id}>
                        <td className="p-3 text-sm text-slate-500">{log.id}</td>
                        ...
                        {log.product.name} <span className="text-slate-400">({log.product.sku})</span>
                        </td>
                        <td className="p-3 text-sm text-slate-500">{log.product.supplier || '-'}</td>
                        <td className="p-3 text-sm text-slate-500 text-right">{stokAwal}</td>
                        <td
                        className={`p-3 text-sm font-bold text-right flex items-center justify-end gap-1 ${
                        ...
                        <td className="p-3 text-sm text-slate-500 text-right">{totalStok}</td>
                        <td className="p-3 text-center">
                        <span
                            className={`px-2.5 py-1 text-xs font-semibold rounded-full ${getLogTypeStyling(
                            log.type
                            )}`}
                        >
                            {log.type.replace('_', ' ')}
                        ...
                        <td
                        className="p-3 text-sm text-slate-500 italic max-w-xs truncate"
                        title={log.notes || ''}
                        >
                        {log.notes || '-'}
                        </td>
                        ...
                        <button
                            ...
                            className="text-sky-600 hover:text-sky-800 p-1.5"
                            title="Edit Catatan"
                        >
                            <Edit size={16} />
                        </button>
