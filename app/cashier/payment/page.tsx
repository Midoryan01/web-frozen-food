    ...
    <>
      ...
      <div id="receipt-print-area" className="hidden print:block">
        <div className="receipt-content">
          ...
          <div className="receipt-header">
            ...
            <p className="store-tagline-sub">
              Makanan Beku Berkualitas & Praktis
            </p>
          </div>
          ...
          <div className="details ">
            <p>
              <strong>No. Order:</strong> {details.orderNumber}
            </p>
            <p>
            ...
            {details.customerName && (
              <p>
                <strong>Pelanggan:</strong> {details.customerName}
              </p>
            )}
          </div>
          <hr />
            ...
            <tbody>
              {details.items.map((item) => (
                <React.Fragment key={item.id}>
                  <tr>
                    <td colSpan={3} className="item-name">
                      {item.name}
                    </td>
                  </tr>
                  <tr>
                    ...
                    <td className="price-total">
                      Rp
                      {(item.sellPrice * item.quantity).toLocaleString("id-ID")}
                    </td>
                  </tr>
                </React.Fragment>
              ))}
          ...
          <div className="totals">
            ...
            {details.paymentMethod.toUpperCase() === "CASH" &&
              typeof details.cashReceived === "number" && (
                <>
                  <div className="totals-row">
                    <span>TUNAI</span>
                    <span>
                      Rp{details.cashReceived.toLocaleString("id-ID")}
                    </span>
                  </div>
                  <div className="totals-row total-change">
                    <span>KEMBALI</span>
                    <span>
                      Rp{(details.changeGiven ?? 0).toLocaleString("id-ID")}
                    </span>
                  </div>
                </>
              )}
          ...
          <br />
          <p className="footer-address text-center text-[8pt] leading-tight">
            Jl. Setia Darma I<br />
            Setiadarma, Tambun Sel.
            <br />
            Bekasi 17510
          </p>
      ...
      <style jsx global>{`
        @media print {
          ...
          .receipt-content .store-tagline {
            font-weight: bold;
            font-size: 10pt;
            margin: 0;
          }
          .receipt-content .store-tagline-sub {
            font-size: 8.5pt;
            margin: 0;
          }
          ...
          .receipt-content .price-detail {
            text-align: left;
          }
          .receipt-content .price-total {
            text-align: right;
            font-weight: 500;
          }
          ...
          .receipt-content .totals-row span:first-child {
            text-align: left;
          }
          .receipt-content .totals-row span:last-child {
            text-align: right;
          }
          ...
          .receipt-content .receipt-footer-date {
            text-align: center;
            font-size: 8pt;
            margin-top: 4mm;
          }
    ...
    <div className="min-h-screen bg-slate-100 flex flex-col items-center justify-center p-4 sm:p-6 lg:p-8">
      <div className="w-full max-w-4xl">
        <header className="mb-6">
          <button
            onClick={() => router.back()}
            className="flex items-center gap-2 text-sky-600 hover:text-sky-700 font-medium transition-colors mb-4 text-sm"
          >
          ...
          <div className="md:col-span-3 bg-white p-6 sm:p-8 rounded-2xl shadow-lg border border-slate-200">
            <section className="mb-6">
              <label
                ...
                className="text-sm font-medium text-slate-700 mb-2 flex items-center gap-2"
              >
            ...
            {/* QRIS (disabled) */}
            {paymentMethod === "QRIS" && (
              <section className="p-4 bg-slate-100 border border-slate-300 rounded-lg text-center">
                <QrCode className="mx-auto text-slate-400 mb-2" size={40} />
                <p className="text-sm text-slate-500">
                  Pembayaran QRIS sedang dalam pengembangan.
                </p>
              </section>
            )}
            ...
            {(paymentMethod === "DEBIT CARD" || paymentMethod === "CREDIT CARD") && (
              <section className="p-4 bg-slate-100 border border-slate-300 rounded-lg text-center">
                <Info className="mx-auto text-slate-400 mb-2" size={40} />
                <p className="text-sm text-slate-500">
                  Pembayaran kartu sedang dalam pengembangan.
                </p>
              </section>
            )}
          </div>
          ...
          <div className="md:col-span-2">
            <div className="bg-white p-6 sm:p-8 rounded-2xl shadow-lg border border-slate-200 h-full flex flex-col">
                ...
                {paymentMethod === "CASH" && parseFloat(cashReceived) > 0 && (
                  <div className="flex justify-between items-center mt-2 text-slate-700">
                    ...
                    <p className="text-lg font-bold">
                      Rp{parseFloat(cashReceived).toLocaleString("id-ID")}
                    </p>
                  </div>
                )}
              ...
              {paymentStatus === "failed" && (
                <p className="text-red-600 text-center text-sm mt-3 flex items-center justify-center gap-2">
